...
/**
 * 顶部英雄区 - AnzhiYu 风格方块走动图
 * 类似 AnzhiYu 主题的 peoplecanvas 效果
 * @returns
 */
const Hero = props => {
  return (
    <div
      id='hero-wrapper'
      className='w-full overflow-hidden select-none px-5 mb-4'>
      <div
        id='hero'
        style={{ zIndex: 1 }}
        className='recent-post-top rounded-[12px] 2xl:px-5 max-w-[86rem] w-full mx-auto relative'>
        {/* AnzhiYu 风格方块走动图 */}
        <PeopleCanvas {...props} />
      </div>
    </div>
...
/**
 * AnzhiYu 风格方块走动图组件
 * 基于 Canvas 实现的动态方块走动效果
 */
function PeopleCanvas(props) {
  ...
  // 方块走动图配置
  const peopleConfig = {
    src: siteConfig('HEO_PEOPLE_CANVAS_IMG', null, CONFIG) || 'https://upload-bbs.miyoushe.com/upload/2024/07/27/125766904/ba62475f396df9de3316a08ed9e65d86_5680958632268053399.png',
    rows: siteConfig('HEO_PEOPLE_CANVAS_ROWS', 15, CONFIG),
    cols: siteConfig('HEO_PEOPLE_CANVAS_COLS', 7, CONFIG),
    height: siteConfig('HEO_PEOPLE_CANVAS_HEIGHT', 300, CONFIG),
  }
  ...
  const initPeopleCanvas = () => {
    ...
    const randomIndex = (arr) => Math.floor(randomRange(0, arr.length))
    const removeRandomFromArray = (arr) => arr.splice(randomIndex(arr), 1)[0]
    const getRandomFromArray = (arr) => arr[randomIndex(arr)]
    
    // 重置方块位置
    const resetPeep = (peepData) => {
      const { stage, peep } = peepData
      const direction = Math.random() > 0.5 ? 1 : -1
      const easeValue = 100 - 250 * (Math.random() ** 2)
      const y = stage.height - peep.height + easeValue
    ...
    // 正常走动动画
    const normalWalk = (peepData) => {
      const { peep, props } = peepData
    ...
    // 创建所有方块
    const createPeeps = () => {
      const { rows, cols } = peopleConfig
      const totalPeeps = rows * cols
      const cellWidth = img.naturalWidth / rows
      const cellHeight = img.naturalHeight / cols
    ...
    const resize = () => {
      ...
      // 清理现有动画
      crowd.forEach(peep => {
        if (peep.walk && peep.walk.kill) peep.walk.kill()
      })
    ...
    const addPeepToCrowd = () => {
      ...
      const walkFunc = getRandomFromArray(walks)
      const walk = walkFunc({
        peep,
  ...
  return (
    <div 
      className="people-canvas-container relative w-full bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-800 dark:to-gray-900 rounded-xl overflow-hidden"
      style={{ height: `${peopleConfig.height}px` }}
    >
      {/* Canvas 方块走动图 */}
      <canvas
        ref={canvasRef}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center text-white">
          <h1 className="text-3xl font-bold mb-2 drop-shadow-lg">
            {siteInfo?.title || 'Welcome'}
          </h1>
          <p className="text-lg opacity-90 drop-shadow-md">
            {siteInfo?.description || 'Explore the digital world'}
          </p>
        </div>
      </div>
